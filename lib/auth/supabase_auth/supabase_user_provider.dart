...
class KfdSupabaseUser extends BaseAuthUser {
  KfdSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> kfdSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = KfdSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
